...
let gCmd = {
  ...
  async openInNewPrivateWnd(aBookmarkID, aURL)
  {
    ...
    try {
      ...
      console.error("CollectionFox::sidebar.js: gCmd.openInNewPrivateWnd(): Error from sidebar context menu: " + e);
    }
  },
...
$(async () => {
  ...
  log(`CollectionFox: Sidebar width ${window.innerWidth} px`);
  ...
  // Show update message bar if CollectionFox was just updated.
...
{
  log("CollectionFox::sidebar.js: initReadingList(): Initializing sidebar" + (aLocalDataOnly ? " (local data only).":"."));
  ...
  if (gPrefs.syncEnabled && !aLocalDataOnly) {
    log("CollectionFox::sidebar.js: initReadingList(): Sync enabled.  Syncing reading list.");
...
{
  log(`CollectionFox: ${aBookmarks.length} items.`);
  log(aBookmarks);
  ...
  gRenameOtherWndMsgBox.onFirstInit = function ()
  {
    this.find("#switch-wnd-btn").on("click", async (aEvent) => {
      if (!this.renameDlgSrcWndID) {
        throw new ReferenceError("CollectionFox::sidebar.js: gRenameOtherWndMsgBox: renameDlgSrcWndID not initialized");
      }
  ...
  gKeybdCxtMenu.onFirstInit = function ()
  {
    ...
    $("#cmd-delete").on("click", async (aEvent) => {
      ...
      try {
        ...
        warn("CollectionFox: Error removing bookmark: " + e);
      }
      this.close();
    });
    ...
    this._dlgElt.on("mouseenter", aEvent => {
      ...
      if (!focusedBtn) {
        warn("CollectionFox::sidebar.js: Nothing focused in the keyboard-activated context menu");
        return;
      }
    ...
    items: {
      ...
      deleteBookmark: {
        ...
        async callback(aKey, aOpt) {
          let bkmkElt = aOpt.$trigger[0];
          let bkmkID = bkmkElt.dataset.id;
          log(`CollectionFox::sidebar.js: Removing bookmark ID: ${bkmkID}`);
          try {
            await gCmd.deleteBookmark(bkmkID);
          }
          catch (e) {
  ...
  $("#reading-list").on("keydown.readingList", aEvent => {
    ...
    if (aEvent.key == "ArrowDown" || isEndKeyPressed(aEvent)) {
      if (gKeybSelectedIdx === null) {
        ...
        warn("CollectionFox::sidebar.js: Reached the end of the reading list.");
      }
      else {
      ...
      if (! gKeybSelectedIdx) {
        warn("CollectionFox::sidebar.js: Reached the start of the reading list.");
      }
      else {
        let item = rdgListItems.get(gKeybSelectedIdx);
        item.classList.remove("focused");
...
browser.runtime.onMessage.addListener(aMessage => {
  log(`CollectionFox::sidebar.js: Window ID ${gWndID} received extension message "${aMessage.id}"`);
...
$(window).on("resize", aEvent => {
  warn("CollectionFox::sidebar.js: The 'resize' event was fired!!");
...
$("#add-link, #add-link-cta").on("click", async (aEvent) => {
  ...
  try {
    ...
    warn("CollectionFox: Error adding bookmark: " + e);
    return;
  }
