...
browser.runtime.onInstalled.addListener(async (aInstall) => {
  if (aInstall.reason == "install") {
    log("CollectionFox: Extension installed.");
  }
    ...
    if (currVer == oldVer) {
      log("CollectionFox: WebExtension reloaded.");
    }
    else {
      log(`CollectionFox: Updating from version ${oldVer} to ${currVer}`);
      ...
      // that CollectionFox was just updated.
...
void async function ()
{
  log("CollectionFox: WebExtension startup initiated.");

  let prefs = await aePrefs.getAllPrefs();
  log("CollectionFox: Successfully retrieved user preferences:");
  log(prefs);

  if (! aePrefs.hasUserPrefs(prefs)) {
    log("Initializing CollectionFox user preferences.");
...
async function init(aPrefs)
{
  ...
  log(`CollectionFox: Host app: ${brws.name} (version ${brws.version})`);
  log(`CollectionFox: OS: ${platform.os}`);
  ...
  if (aPrefs.syncEnabled) {
    info("CollectionFox: Synced reading list is enabled.");
...
async function firstSyncReadingList()
{
  ...
  log("CollectionFox: Starting first reading list sync...");
  await aeSyncReadingList.firstSync();
  log("CollectionFox: Finished first sync!");
...
async function syncReadingList()
{
  ...
  if (isPaused) {
    info("CollectionFox: syncReadingList(): Syncing is paused. Loading reading list data from the database.");
  }
  else {
    ...
    log("CollectionFox: Starting reading list sync...");
    ...
    try {
      ...
      if (e instanceof aeAuthorizationError) {
        warn("CollectionFox: syncReadingList(): Caught aeAuthorizationError exception.  Details:\n" + e);
        await handleAuthorizationError();
        throw e;
      }
      else if (e instanceof aeNotFoundError) {
        warn("CollectionFox: syncReadingList(): Caught aeNotFoundError exception.  Details:\n" + e);
        log("Regenerating sync file...");
        await aeSyncReadingList.push(true);
      }
      else if (e instanceof TypeError) {
        warn("CollectionFox: syncReadingList(): Caught TypeError exception.  Unable to connect to the cloud file host.  Details:\n" + e);
        await handleNetworkConnectError();
        throw e;
      }
      else {
        console.error("CollectionFox: syncReadingList(): An unexpected error has occurred.  Details:\n" + e);
        throw e;
      }
    }
    
    log("CollectionFox: Finished sync!");
  }
...
async function handleAuthorizationError()
{
  ...
  if (syncAlarm) {
    log("CollectionFox: handleAuthorizationError(): Suspending auto sync interval.");
...
function initSyncInterval(aPrefs)
{
  ...
  info(`CollectionFox: Reading list will be synced every ${periodInMinutes} mins.`);
}
...
async function pauseSync(aIsPaused=true)
{
  ...
  log(`CollectionFox: It is ${aIsPaused} that reading list sync is paused.`);
}
...
async function stopSync()
{
  ...
  log("CollectionFox: Sync stopped.");
}
...
{
  log("CollectionFox: Restarting sync interval...");
  await browser.alarms.clear("sync-reading-list");
...
async function pushLocalChanges()
{
  let syncEnabled = await aePrefs.getPref("syncEnabled");
  if (syncEnabled) {
    log("CollectionFox: Pushing local changes...");
    try {
      await aeSyncReadingList.push();
    }
    catch (e) {
      if (e instanceof aeAuthorizationError) {
        warn("CollectionFox: pushLocalChanges(): Caught aeAuthorizationError exception.  Details:\n" + e);
        await handleAuthorizationError();
      }
      else {
        console.error("CollectionFox: pushLocalChanges(): An unexpected error has occurred.  Details:\n" + e);
      }
      throw e;
    }
...
async function getFileHostUserInfo()
{
  ...
  if (syncEnabled) {
    try {
      ...
      if (e instanceof aeAuthorizationError) {
        warn("CollectionFox: getFileHostUserInfo(): Caught aeAuthorizationError exception.  Details:\n" + e);
        await handleAuthorizationError();
      }
      else if (e instanceof TypeError) {
        warn("CollectionFox: getFileHostUserInfo(): Caught TypeError exception.  Unable to connect to the cloud file host.  Details:\n" + e);
      }
      else {
        console.error("CollectionFox: getFileHostUserInfo(): An unexpected error has occurred.  Details:\n" + e);
      }
      throw e;
    }
  }
...
async function setBookmarkFavIcon(aBookmarkID, aFavIconDataURL)
{
  if (!aFavIconDataURL || !aFavIconDataURL.startsWith("data:")) {
    log("CollectionFox: setBookmarkFavIcon(): No favicon data found for bookmark " + aBookmarkID);
    return;
  }
...
browser.runtime.onMessage.addListener(aMessage => {
  log(`CollectionFox: Background script received extension message "${aMessage.id}"`);
  ...
  case "sync-setting-changed":
    if (aMessage.syncEnabled) {
      warn("CollectionFox: Sync turned ON from extension preferences.");
      return firstSyncReadingList();
    }
    else {
      warn("CollectionFox: Sync turned OFF.");
      stopSync().then(() => {
...
browser.windows.onFocusChanged.addListener(async (aWndID) => {
  ...
  if (wnd.id == aWndID) {
    log(`CollectionFox: Handling window focus changed event for window ${wnd.id}`);
    ...
    if (syncEnabled) {
      ...
      log("CollectionFox: Syncing reading list.")
      try {
        await syncReadingList();
      }
      catch {}
...
browser.tabs.onUpdated.addListener(async (aTabID, aChangeInfo, aTab) => {
  if (aChangeInfo.status == "complete") {
    ...
    let [actvTab] = await browser.tabs.query({active: true, currentWindow: true});
    log(`CollectionFox: browser.tabs.onUpdated(): Handler called for tab ${aTabID}\nTab ${actvTab.id} is the active tab in the current window`);
...
browser.tabs.onActivated.addListener(async (aActiveTab) => {
  ...
  if (tab.url) {
    ...
    warn("CollectionFox: Tab 'onActivated' event handler is unable to retrieve URL for tab " + aActiveTab.tabId);
  }
});
    ...
    for (let i = 0; i < selectedTabs.length; i++) {
      ...
      if (isSupportedURL(tab.url) && !isRestrictedURL(tab.url)) {
        ...
        if (selectedTabs.length == 1) {
          ...
          warn("CollectionFox: Unsupported page won't be added to reading list: " + tab.url);
        }
      }
    }
  }
